...
const ReportCalendarStrip = ({ value, onChange }) => {

    let val = value || moment()  //get data from main calender.js file
    const [selected, setSelected] = useState(val) // date selected month and year store state
    const [currentDay, setCurrentDay] = useState({ day: val?.format('dd'), date: val?.format('DD') }) // current date

    const flRef = React.useRef() //for current date scroll 
    ...
    //active date on selection
    const isActive = (item) => {
        return currentDay.date === item.date
    }
    //calender selected date handle 
    ...
    var all_days = getdDaysInMonth(selected) // all days from selected months

    // onselected date checking conditionif greater than 0
    useEffect(() => {
...
export default ReportCalendarStrip